...
data class SearchRequest(
    ...
    init {
        floorRequirements.floorValidationProblem(maximumDepth)?.let { throw IllegalArgumentException(it) }
        requirements.validationProblem(arcaneResin, arcaneResinAuto, floorRequirements.isNotEmpty())?.let { throw IllegalArgumentException(it) }
    ...
    val slotCount: Int
        get() = requirements.slotCount() + floorRequirements.size + if (arcaneResinAuto || arcaneResin > 0) 1 else 0
}
