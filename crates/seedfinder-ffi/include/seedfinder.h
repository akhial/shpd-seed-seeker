...
// Finite-list filter requests use SFF1: magic[4], query_length:u16 big-endian,
// nested SSF7 query, seed_count:u16 big-endian, then 11 ASCII bytes per seed.
// Filter responses use the existing SSR1 seed-only result packet.
...
int32_t seedfinder_filter(const uint8_t *request, size_t request_len, uint8_t **out_packet, size_t *out_len);
