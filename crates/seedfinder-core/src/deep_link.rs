...
//! ever grow at the end. Versions 4 through 14 are supported; versions 1
...
/// Version 14 adds a four-bit artifact transmutation count after the trinket count.
...
pub fn encode(query: &SearchQuery) -> Result<String, String> {
    ...
    let version = encoding_version(query);
    bits.push(version, 4);
    if version >= 6 {
    ...
    for requirement in &query.requirements {
        ...
        if version >= 14 {
            bits.push(requirement.artifact_transmutations.into(), 4);
        }
    }
    if version >= 11 {
...
fn encoding_version(query: &SearchQuery) -> u32 {
        ...
        || query.requirements.iter().any(|r| r.exclude_resin)
    {
        ...
        9
    } else if query.arcane_resin_filter != ArcaneResinFilter::default() {
        8
    } else if query.arcane_resin > 0 {
        7
    } else if query.auto_apply_trinket {
        6
    } else if query.requirements.iter().any(|r| r.select_trinket) {
        5
    } else {
        VERSION.into()
    }
...
#[allow(clippy::too_many_lines)] // Read each versioned field in wire order.
pub fn decode(code: &str) -> Result<SearchQuery, String> {
    let bytes = base64url_decode(code.trim())?;
    let mut bits = BitReader::new(&bytes);
    let version = bits.pull(4)?;
    if !(u32::from(VERSION)..=14).contains(&version) {
        return Err(format!(
            "this link uses format version {version}; this app only understands \
             versions {VERSION} through 14 — it may have been created by a different release"
        ));
    }
        ...
        .map(|index| {
            decode_requirement(&mut bits)
                .and_then(|mut requirement| {
                    ...
                    if version >= 14 {
                        requirement.artifact_transmutations = bits.pull(4)? as u8;
                    }
                    Ok(requirement)
                })
...
mod tests {
    ...
    fn rejects_malformed_codes() {
        ...
        // Unsupported future version (bits 1111 in the top nibble).
        assert!(decode("8AAA").unwrap_err().contains("version 15"));
    ...
    fn retired_versions_are_rejected() {
        for code in ["EAGWhMA", "IAIQ4sCAEWJAgA", "MAGWhMAA"] {
            ...
            assert!(
                error.contains("only understands versions 4 through 14"),
                "{error}"
            );
        }
    }
