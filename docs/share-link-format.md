...
| Field | Bits | Meaning |
| --- | --- | --- |
| `version` | 4 | `4` for ordinary queries, `5` when a requirement selects a trinket, `6` when auto-apply is enabled, `7` for an Arcane Resin minimum, `8` for resin donor filters, `9` when a requirement is a blanket, `10` for Auto resin (including blanket bits). `11` for exact-floor requirements, `12` for a starting Mage wand credit or per-wand Auto exclusion. `13` for trinket transmutation limits, `14` for artifact transmutation limits. Decoders accept 4–14; retired versions 1–3 and future versions are rejected. Existing tables and layouts are frozen. |
| `auto_apply_trinket` | 1, versions 6–14 only | Immediately follows the version nibble. Boolean query flag; absent in versions 4–5, which default to false. |
...
| `arcane_resin` | 16, versions 7–14 only | Minimum resin from surplus donor wands; zero disables resin in versions 7–9. Version 10 implies Auto and writes this numeric field as zero. Absent in versions 4–6. |
| resin `uncursed` | 1, versions 8–14 only | Whether donors must be uncursed; defaults to true in earlier formats. |
| resin `source` | 1 (+5), versions 8–14 only | Optional donor source using the same frozen source table. |
| resin `max_depth` | 1 (+5), versions 8–14 only | Optional donor floor limit, stored as `depth − 1`. |
| resin `include_mage_wand` | 1, versions 12–14 | Credit 2 resin from the preserved starting Magic Missile wand; independent of donor filters. |
...
| `select_trinket` | 1, versions 5–14 only | Whether this requirement selects its offered trinket. Version 6 includes this bit even when auto-apply is disabled by explicit trinket requirements. |
| `blanket` | 1, versions 9–14 only | Follows `select_trinket`; requires an item assigned to an ordinary slot or selected as an Arcane Resin donor to also satisfy this predicate. Absent in versions 4–8, which default to false. |
| `exclude_resin` | 1, versions 12–14 | Follows `blanket`; omit this ordinary wand’s Auto upgrade cost while keeping it reserved. Invalid on blankets and other item kinds. |
| `trinket_transmutations` | 4, versions 13–14 | Follows `exclude_resin`; maximum 0–13, default 0 in older links. |
| `artifact_transmutations` | 4, version 14 | Follows the trinket limit; maximum 0–10, default 0 in older links. |
...
This follows all requirement records. Versions 12–14 retain version 11’s layout.

| Field | Bits | Meaning |
| --- | --- | --- |
| `arcane_resin_auto` | 1 | Explicit Auto mode; Auto writes the numeric resin field as zero. |
| floor requirement count | 5 | Number of exact-floor requirements. |
| `depth` | 5 per floor | Actual floor number, 1–24; boss floors are invalid. |
| `feeling` | 1 (+3) per floor | Optional feeling using the frozen game feeling IDs. |
| `rooms` | 7 (+7 per room) per floor | Count and IDs of required room types, deduplicated in enum order. |
| `any_rooms` | 7 (+7 per room) per floor | Count and IDs of alternative room types, in the same encoding. |
