...
public sealed partial class MainWindow : Window
{
    ...
    private async Task<bool> EditRequirement(ItemRequirement r, bool isNew)
    {
        ...
        var content = new StackPanel { Spacing = 12, Padding = new Thickness(2, 4, 2, 4) }; foreach (var control in new UIElement[] { kind, item, tierMatch, tier, upgradeMatch, upgrade, modifier, source, group, depthRow, depth }) content.Children.Add(control);
        void SyncVisibility()
        {
        ...
        kind.SelectionChanged += (_, _) => { r.Item = null; r.Modifier = null; Populate(); }; item.SelectionChanged += (_, _) => SyncVisibility(); tierMatch.SelectionChanged += (_, _) => SyncVisibility(); upgradeMatch.SelectionChanged += (_, _) => SyncVisibility(); depthToggle.Toggled += (_, _) => depth.Visibility = depthToggle.IsOn ? Visibility.Visible : Visibility.Collapsed;
        Populate(); depth.Visibility = depthToggle.IsOn ? Visibility.Visible : Visibility.Collapsed;
        ...
        r.Source = source.SelectedIndex == 0 ? null : (ScoutItemSource)(source.SelectedIndex - 1); r.IdentityGroup = group.SelectedIndex == 0 ? null : group.SelectedIndex; r.MaximumDepth = depthToggle.IsOn ? (int)depth.Value : null; return true;
    }
