    ...
    public static func decode(_ packet: Data) throws -> ScoutWorld {
        ...
        if magic == Data("SSC8".utf8) {
            ...
            for _ in 0..<count {
                let depth = Int(try input.u8())
                guard (1...24).contains(depth), depth % 5 != 0, depth > previousDepth else {
                    throw WireCodecError.invalidValue("Room depths must be ascending regular floors 1..24")
                }
                previousDepth = depth
                var rooms = Set<String>()
                for _ in 0..<Int(try input.u8()) {
                    let room = try input.utf8(input.u16())
                    guard !room.isEmpty, rooms.insert(room).inserted else {
                        throw WireCodecError.invalidValue("Empty or repeated floor room")
                    }
                }
